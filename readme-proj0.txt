...
(2) Try running the program immediately again when it finishes
    ...
    If run multiple times, immediately the client will raise a Connection refused
    error because the Server reports an Address already in use error. When we
    remove the both sleep conditions, the program creates the client socket
    prints done, then raises a Connection refused exception.
